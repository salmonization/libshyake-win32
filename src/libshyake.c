...
base64_decode(const char *b64, size_t *out_len)
{
    ...
    /* normalize urlsafe alphabet and re-pad to a multiple of 4 */
    char *std_b64 = malloc(len + 4);
    if (!std_b64) { free(out); return NULL; }
    memcpy(std_b64, b64, len);
    ...
    size_t dec_len;
    int rc = mbedtls_base64_decode(out, len ? len : 1, &dec_len,
                                   (const unsigned char *)std_b64,
                                   padded);
    free(std_b64);
...
shyake_free_ctx(shyake_ctx *ctx)
{
    ...
    free(ctx->username);
    SecureZeroMemory(ctx->passphrase, sizeof(ctx->passphrase));
    SecureZeroMemory(ctx->new_passphrase, sizeof(ctx->new_passphrase));
    free(ctx);
    curl_global_cleanup();
}
